Weather prediction is essential to reduce the impact caused by natural disasters and to effectively manage natural resources like water. 
%It will give more understanding of weather prediction if we look into a weather prediction workflow that can use to predict the surface water level. 
...
% \db{For example, FLO2D is a hydrologic model that can use to predict the surface water level. A given FLO2D model can simulate with the initial surface water levels and the sequence of forecasted rainfall for the corresponding modeled area from the \acrshort{nwm} model to forecast future water levels.}
\dbc{Highighted part doesn't make much sense to me. Need to be reworded.}
\gkc{Highlighted parts modified and kept also.}
\dbc{Highighted part is still not clear to me.}
\gkc{Removed the sentence. I tried to use the FLO2D as a model which can use WRF output to forecast water levels.}
...
\dbc{Something seems to be wrong with reference Macdermaid2005ARCHITECTUREP2.39}
\gkc{Seems to be issue with Mendeley.}
\dbc{"Island like Sri Lanka which experience different kinds of weather seasons over the year" We need to add a citation that talks about multiple types of weather patterns in Sri Lanka} 
Island like Sri Lanka which experience different kinds of weather seasons over the year \cite{NaveendrakumarFiveLanka}.
\gkc{Added a reference.}
...
\db{Given the above facts, there is a necessity to develop a WDIA system that could capable of handling large multimodel data efficiently with providing scalability and high throughput with using open source tools which support modern cloud computing advancement, as well as support data sharing with other NWMs and third-party access.}
\dbc{Complete the above sentence by adding a list of desirable features.}
