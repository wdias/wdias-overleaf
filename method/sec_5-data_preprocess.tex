...
\gkc{During spatial interpolation, we use interpolation to either fill gaps in timeseries or to create a new timeseries for a location using data from other (spatially distributed) locations' timeseries. Spatial interpolation can also be applied for sampling scalar timeseries from grid timeseries, for re-sampling grids, or for creating grids from timeseries data. As an example, let’s assume that there are precipitation timeseries data available for the location A and location B as shown in Figure x. Then we want to calculate the mean precipitation of nearby location using these two timeseries. We can use the spatial interpolation to get the mean precipitation of nearby location.}
\begin{figure}[htp]
    \centering
...
\gkc{Simple arithmetic manipulation of time interval transformation and shifting the series in time-specific hydro-meteorological transformation such as stage-discharge relationships. As an example, there can be a timeseries of precipitation with 1-minute sample data. There will be a model requirement that needs hourly data. So, we need to do accumulation data manipulation to create a timeseries of hourly data using 1-minute sample data.}
\begin{figure}[htp]
    \centering
...
\gkc{Checks for counting reliable, doubtful, unreliable, and missing values. The weather stations can produce invalid observations due to technical errors in the sensors of the devices. If we feed those values directly to the models, the models will produce inaccurate forecasting. To avoid that, we need to modify or flag those values before feeding them to the numerical models. We can use validation extensions to modify or flag invalid data. As an example, a weather station can record temperature above 70 celsius due to sensor error or calibration issues. But in real-world, it is not possible to have such a temperature in nature. Thus we need to flag such temperature issues and need to repair the weather station as soon as possible.}
\begin{figure}[htp]
    \centering
